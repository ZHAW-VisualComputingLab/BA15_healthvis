...
function dataProcessingSite5(){
    ...
    // Set dataheadername
    viualizationSite5Options.lineChart.setDataHeaderName(d3.entries(viualizationSite5Options.loadedFile[0]).sort(sortYearsAscNameFirst)[0].key);
    ...
    for(var k = 0; k<viualizationSite5Options.loadedFile.length; k++){
        tempTimeLineName = d3.entries(viualizationSite5Options.loadedFile[k]).sort(sortYearsAscNameFirst)[0].value;
        tempConvertedData = d3.entries(viualizationSite5Options.loadedFile[k]);
        // Remove non-numeric data values
